...
/**
 * <p>
 * This class is <u>not</u> Thread safe. Use in single threaded contexts, or
 * contexts where single threaded-access can be guaranteed, or wrap with
...
public class DoubleWeakHashMap implements Map {
	...
	/*
	 * public static void main(String[] argv) { DoubleWeakHashMap m = new
	 * DoubleWeakHashMap(); //Set keySet = new HashSet(); //Set valSet = new
	 * HashSet();
	 * 
	 * while (true) { System.err.println( m.inner.size() );
	 * 
	 * //if (Math.random() < 0.1f) // valSet.clear();
	 * 
	 * Object key = new Object(); Object val = new long[100000];
	 * //keySet.add(key); //valSet.add(val); m.put( key, val ); } }
	 */
}
