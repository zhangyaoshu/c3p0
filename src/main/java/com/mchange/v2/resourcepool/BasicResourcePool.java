...
class BasicResourcePool implements ResourcePool {
	 ...
	 *            may be null
	 */
	public BasicResourcePool(Manager mgr, int start, int min, int max, int inc, int num_acq_attempts, int acq_attempt_delay, long check_idle_resources_delay, long max_resource_age, long max_idle_time, long excess_max_idle_time, long destroy_unreturned_resc_time, long expiration_enforcement_delay, boolean break_on_acquisition_failure, boolean debug_store_checkout_exceptions,
			AsynchronousRunner taskRunner, RunnableQueue asyncEventQueue, Timer cullAndIdleRefurbishTimer, BasicResourcePoolFactory factory) throws ResourcePoolException {
		try {
			this.mgr = mgr;
			this.start = start;
			this.min = min;
			...
			if (mustEnforceExpiration()) {
				...
				// System.err.println("minExpirationTime(): " +
				// minExpirationTime());
				// System.err.println("this.expiration_enforcement_delay: " +
			...
			// System.err.println("this.check_idle_resources_delay: " +
			// this.check_idle_resources_delay);
			...
			if (logger.isLoggable(MLevel.FINER))
				logger.finer(this + " config: [start -> " + this.start + "; min -> " + this.min + "; max -> " + this.max + "; inc -> " + this.inc + "; num_acq_attempts -> " + this.num_acq_attempts + "; acq_attempt_delay -> " + this.acq_attempt_delay + "; check_idle_resources_delay -> " + this.check_idle_resources_delay + "; mox_resource_age -> " + this.max_resource_age + "; max_idle_time -> "
						+ this.max_idle_time + "; excess_max_idle_time -> " + this.excess_max_idle_time + "; destroy_unreturned_resc_time -> " + this.destroy_unreturned_resc_time + "; expiration_enforcement_delay -> " + this.expiration_enforcement_delay + "; break_on_acquisition_failure -> " + this.break_on_acquisition_failure + "; debug_store_checkout_exceptions -> "
						+ this.debug_store_checkout_exceptions + "]");

		} catch (Exception e) {
			// if ( logger.isLoggable( MLevel.WARNING) )
			// logger.log( MLevel.WARNING,
			// "Could not create resource pool due to Exception!", e );
	...
	private boolean mustEnforceExpiration() {
		return max_resource_age > 0 || max_idle_time > 0 || excess_max_idle_time > 0 || destroy_unreturned_resc_time > 0;
	}
	...
	private void _recheckResizePool() {
		...
		if (!broken) {
			...
			// pending_acquires + "; pending_removes: " + pending_removes +
			// ')');
			// new Exception( "_recheckResizePool() STACK TRACE"
			// ).printStackTrace();
	...
	private void expandPool(int count) {
		...
		// XXX: temporary switch -- assuming no problems appear, we'll get rid
		// of AcquireTask
	...
	private synchronized Object prelimCheckoutResource(long timeout) throws TimeoutException, ResourcePoolException, InterruptedException {
		try {
			...
			if (available == 0) {
				...
				if (msz < max) {
					// to cover all the load, we need the current size, plus
					// those waiting already for acquisition,
					...
					if (desired_target >= target_pool_size) {
						// make sure we don't grab less than inc Connections at
						// a time, if we can help it.
			...
			// this is a hack -- but "doing it right" adds a lot of complexity,
			// and collisions between
			// an idle check and a checkout should be relatively rare. anyway,
			...
			if (idleCheckResources.contains(resc)) {
				...
				// we'll move remove() to after the if, so we don't have to add
				// back
				// unused.add(0, resc );

				// we'll wait for "something to happen" -- probably an idle
				// check to
		...
		} catch (ResourceClosedException e) // one of our async threads died
		{
			...
			// " -- the pool was found to be closed or broken during an attempt to check out a resource.");
			// e.printStackTrace();
			...
			// System.err.println(this +
			// " -- an attempt to checkout a resource was interrupted: some other thread "
			// +
	...
	// this is designed as a response to a determination that our resource
	// source is down.
	// rather than declaring ourselves broken in this case (as we did
	// previously), we
	// kill all pending acquisition attempts, but retry on new acqusition
	// requests.
	private synchronized void forceKillAcquires() throws InterruptedException {
		...
		try {
			...
			this.notifyAll(); // wake up any threads waiting on an acquire, and
								// force them all to die.
			while (acquireWaiters.size() > 0) // we want to let all the waiting
												// acquires die before we unset
												// force_kill_acquires
			{
				otherWaiters.add(t);
				this.wait();
	...
	private void asyncFireResourceRemoved(final Object resc, final boolean checked_out_resource, final int pool_size, final int available_size, final int removed_but_unreturned_size) {
		if (canFireEvents()) {
			...
			Runnable r = new Runnable() {
				public void run() {
					rpes.fireResourceRemoved(resc, checked_out_resource, pool_size, available_size, removed_but_unreturned_size);
				}
			};
			asyncEventQueue.postRunnable(r);
		}
	...
	private void destroyResource(final Object resc, boolean synchronous) {
		class DestroyResourceTask implements Runnable {
			public void run() {
				try {
					...
					// System.err.println("Failed to destroy resource: " +
					// resc);
					// e.printStackTrace();
				}
			}
		...
		if (synchronous || broken) // if we're broken, our taskRunner may be
									// dead, so we destroy synchronously
		{
			if (logger.isLoggable(MLevel.FINEST) && !broken && Boolean.TRUE.equals(ThreadUtils.reflectiveHoldsLock(this)))
				logger.log(MLevel.FINEST, this + ": Destroyiong a resource on an active pool, synchronousy while holding pool's lock! " + "(not a bug, but a potential bottleneck... is there a good reason for this?)", new Exception("DEBUG STACK TRACE"));
	...
	private void doAcquire() throws Exception {
		...
		Object resc = mgr.acquireResource(); // note we acquire the resource
												// while we DO NOT hold the
												// pool's lock!
		...
		synchronized (this) // assimilate resc if we do need it
		{
			...
			// "acquired new resource, total_acquired: " + total_acquired);
		...
		if (destroy) {
			mgr.destroyResource(resc); // destroy resc if superfluous, without
	...
	public synchronized void setTargetPoolSize(int sz) {
		if (sz > max) {
			throw new IllegalArgumentException("Requested size [" + sz + "] is greater than max [" + max + "].");
		} else if (sz < min) {
			throw new IllegalArgumentException("Requested size [" + sz + "] is less than min [" + min + "].");
		}
	...
	// }
	// throw new
	// RuntimeException("Internal Error -- the pool determined that it did have a resource available for checkout, but was unable to find one.");
	// }
	...
	public synchronized void close(boolean close_checked_out_resources) {
		if (!broken) // ignore repeated calls to close
		{
			// DEBUG
			// firstClose = new
			// Exception("First close() -- debug stack trace [CRAIG]");
			...
			// we destroy resources asynchronously, but with a dedicated one-off
			// Thread, rather than
			// our asynchronous runner, because our asynchrous runner may be
			// shutting down. The
			// destruction is asynchrounous because destroying a resource might
			// require the resource's
			// lock, and we already have the pool's lock. But client threads may
			// well have the resource's
			// lock while they try to check-in to the pool. The async
			// destruction of resources avoids
			...
			Thread resourceDestroyer = new Thread("Resource Destroyer in BasicResourcePool.close()") {
				public void run() {
					for (Iterator ii = cleanupResources.iterator(); ii.hasNext();) {
						try {
							Object resc = ii.next();
							// System.err.println("Destroying resource... " +
							// resc);
	...
	private void doCheckinManaged(final Object resc) throws ResourcePoolException {
		...
		if (unused.contains(resc)) {
			...
			removeResource(resc, true); // synchronous... if we're broken, async
										// tasks might not work
		else {
			class RefurbishCheckinResourceTask implements Runnable {
				public void run() {
					...
					synchronized (BasicResourcePool.this) {
						...
						if (resc_okay && card != null) // we have to check that
														...
														// in the pool
						{
							...
							if (card != null)
								card.checkout_time = -1; // so we don't see this
															// as still checked
															// out and log an
															// overdue cxn in
															// removeResource()
	...
	private void awaitAvailable(long timeout) throws InterruptedException, TimeoutException, ResourcePoolException {
		...
		try {
			...
			if (Debug.DEBUG && Debug.TRACE == Debug.TRACE_MAX) {
				if (logger.isLoggable(MLevel.FINE))
					logger.fine("awaitAvailable(): " + (exampleResource != null ? exampleResource : "[unknown]"));
				trace();
			}
	...
	private void assimilateResource(Object resc) throws Exception {
		...
		unused.add(0, resc);
		// System.err.println("assimilate resource... unused: " +
		// unused.size());
	...
	private void removeResource(Object resc, boolean synchronous) {
		...
		if (pc != null) {
			if (pc.checkout_time > 0 && !broken) // this is a checked-out
													// resource in an active
													// pool, must be overdue if
													// we are removing it
			{
				if (logger.isLoggable(MLevel.INFO)) {
	...
	private void removeTowards(int new_sz) {
		...
		int count = 0;
		for (Iterator ii = cloneOfUnused().iterator(); ii.hasNext() && count < num_to_remove; ++count) {
			Object resc = ii.next();
			removeResource(resc);
		}
	}
	...
	private void cullExpired() {
		...
		// if we do not time-out checkedout resources, we only need to test
		// unused resources
		...
		for (Iterator ii = checkMe.iterator(); ii.hasNext();) {
			...
			if (shouldExpire(resc)) {
				...
				target_pool_size = Math.max(min, target_pool_size - 1); // expiring
																		// a
																		// resource
																		// resources
																		// the
		...
		if (pc.checkout_time < 0) // resource is not checked out
		{
			...
			if (excess_max_idle_time > 0) {
				...
				if (expired && logger.isLoggable(MLevel.FINER))
					logger.log(MLevel.FINER, "EXPIRED excess idle resource: " + resc + " ---> idle_time: " + idle_age + "; excess_max_idle_time: " + excess_max_idle_time + "; pool_size: " + msz + "; min_pool_size: " + min + " [" + this + "]");
			}
			if (!expired && max_idle_time > 0) {
				expired = idle_age > max_idle_time;
				if (expired && logger.isLoggable(MLevel.FINER))
					logger.log(MLevel.FINER, "EXPIRED idle resource: " + resc + " ---> idle_time: " + idle_age + "; max_idle_time: " + max_idle_time + " [" + this + "]");
			}
			if (!expired && max_resource_age > 0) {
				...
				if (expired && logger.isLoggable(MLevel.FINER))
					logger.log(MLevel.FINER, "EXPIRED old resource: " + resc + " ---> absolute_age: " + abs_age + "; max_absolute_age: " + max_resource_age + " [" + this + "]");
			}
	...
	private void trace() {
		...
		if (logger.isLoggable(MLevel.FINEST)) {
			String exampleResStr = (exampleResource == null ? "" : " (e.g. " + exampleResource + ")");
			logger.finest("trace " + this + " [managed: " + managed.size() + ", " + "unused: " + unused.size() + ", excluded: " + excluded.size() + ']' + exampleResStr);
		}
	}
	...
	class ScatteredAcquireTask implements Runnable {
		...
		private ScatteredAcquireTask(int attempts_remaining, boolean first_attempt) {
			...
			if (first_attempt) {
				...
				if (logger.isLoggable(MLevel.FINEST))
					logger.finest("Starting acquisition series. Incremented pending_acquires [" + pending_acquires + "], " + " attempts_remaining: " + attempts_remaining);
			} else {
				if (logger.isLoggable(MLevel.FINEST))
					logger.finest("Continuing acquisition series. pending_acquires [" + pending_acquires + "], " + " attempts_remaining: " + attempts_remaining);
			}
		}
		...
		public void run() {
			try {
				...
				if (logger.isLoggable(MLevel.FINEST))
					logger.finest("Acquisition series terminated " + (fkap ? "because force-kill-acquires is pending" : "successfully") + ". Decremented pending_acquires [" + pending_acquires + "], " + " attempts_remaining: " + attempts_remaining);
			} catch (Exception e) {
				...
				if (attempts_remaining == 0) // last try in a round...
				{
					decrementPendingAcquires();
					if (logger.isLoggable(MLevel.WARNING)) {
						logger.log(MLevel.WARNING, this + " -- Acquisition Attempt Failed!!! Clearing pending acquires. " + "While trying to acquire a needed new resource, we failed " + "to succeed more than the maximum number of allowed " + "acquisition attempts (" + num_acq_attempts + "). " + "Last acquisition attempt exception: ", e);
					}
					if (break_on_acquisition_failure) {
						// System.err.println("\tTHE RESOURCE POOL IS PERMANENTLY BROKEN!");
						if (logger.isLoggable(MLevel.SEVERE))
							logger.severe("A RESOURCE POOL IS PERMANENTLY BROKEN! [" + this + "] " + "(because a series of " + num_acq_attempts + " acquisition attempts " + "failed.)");
						unexpectedBreak();
					} else {
						try {
							forceKillAcquires();
							...
							if (logger.isLoggable(MLevel.WARNING))
								logger.log(MLevel.WARNING, "Failed to force-kill pending acquisition attempts after acquisition failue, " + " due to an InterruptedException!", ie);

							// we might still have clients waiting, so we should
							// try
							// to ensure there are sufficient connections to
							// serve
							recheckResizePool();
						}
					}
					if (logger.isLoggable(MLevel.FINEST))
						logger.finest("Acquisition series terminated unsuccessfully. Decremented pending_acquires [" + pending_acquires + "], " + " attempts_remaining: " + attempts_remaining);
				} else {
					// if attempts_remaining < 0, we try to acquire forever, so
					// the end-of-batch
					// log message below will never be triggered if there is a
					// persistent problem
					// so in this case, it's better flag a
					// higher-than-debug-level message for
					// each failed attempt. (Thanks to Eric Crahen for calling
					// attention to this
					// issue.)
	...
	/*
	 * task we post to separate thread to acquire pooled resources
	 */
	class AcquireTask implements Runnable {
		...
		public void run() {
			try {
				...
				for (int i = 0; shouldTry(i); ++i) {
					try {
						...
						// end the whole task on interrupt, regardless of
						// success
						// or failure
						throw e;
					} catch (Exception e) {
						...
						// if num_acq_attempts <= 0, we try to acquire forever,
						// so the end-of-batch
						// log message below will never be triggered if there is
						// a persistent problem
						// so in this case, it's better flag a
						// higher-than-debug-level message for
						// each failed attempt. (Thanks to Eric Crahen for
				...
				if (!success) {
					if (logger.isLoggable(MLevel.WARNING)) {
						logger.log(MLevel.WARNING, this + " -- Acquisition Attempt Failed!!! Clearing pending acquires. " + "While trying to acquire a needed new resource, we failed " + "to succeed more than the maximum number of allowed " + "acquisition attempts (" + num_acq_attempts + "). " + (lastException == null ? "" : "Last acquisition attempt exception: "), lastException);
					}
					if (break_on_acquisition_failure) {
						// System.err.println("\tTHE RESOURCE POOL IS PERMANENTLY BROKEN!");
						if (logger.isLoggable(MLevel.SEVERE))
			...
			} catch (ResourceClosedException e) // one of our async threads died
			{
				...
				unexpectedBreak();
			} catch (InterruptedException e) // from force kill acquires, or by
												// the thread pool during the
												// long task...
			{
				if (logger.isLoggable(MLevel.WARNING)) {
					logger.log(MLevel.WARNING, BasicResourcePool.this + " -- Thread unexpectedly interrupted while performing an acquisition attempt.", e);
				}

				// System.err.println(BasicResourcePool.this +
				// " -- Thread unexpectedly interrupted while waiting for stale acquisition attempts to die.");
				// e.printStackTrace();
		...
		private boolean shouldTry(int attempt_num) {
			...
			return !success && !isForceKillAcquiresPending() && (num_acq_attempts <= 0 || attempt_num < num_acq_attempts);
		}
	}

	/*
	 * task we post to separate thread to remove unspecified pooled resources
	 * 
	 * TODO: do removal and destruction synchronously but carefully not
	 * synchronized during the destruction of the resource.
	 */
	...
	class CheckIdleResourcesTask extends TimerTask {
		public void run() {
			try {
				// System.err.println("c3p0-JENNIFER: refurbishing idle resources - "
				// + new Date() + " [" + BasicResourcePool.this + "]");
				if (Debug.DEBUG && Debug.TRACE >= Debug.TRACE_MED && logger.isLoggable(MLevel.FINER))
	...
	class AsyncTestIdleResourceTask implements Runnable {
		...
		public void run() {
			Thread.holdsLock(BasicResourcePool.this);
			...
			try {
				try {
					...
					synchronized (BasicResourcePool.this) {
						if (managed.keySet().contains(resc)) // resc might have
																// been culled
																// as expired
																// while we
																// tested
