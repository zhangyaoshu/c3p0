...
public class DynamicPooledDataSourceManagerMBean implements DynamicMBean {
	...
	private synchronized Exception reinitialize() {
		try {
			// for ComboPooledDataSource, everything we care about is exposed
			// via the PooledDataSource
			// for other implementations, we have to pay attention to nested
			// DataSources
			if (!(pds instanceof ComboPooledDataSource) && pds instanceof AbstractPoolBackedDataSource) {
				if (this.cpds instanceof WrapperConnectionPoolDataSource) // implies
																			// non-null,
																			// this
																			// is
																			// a
			...
			// we need to reregister when the attributes we support may have
			// changed, to be sure
			...
			try {
				...
				if (logger.isLoggable(MLevel.WARNING))
					logger.log(MLevel.WARNING, "An Exception occurred while registering/reregistering mbean " + mbeanName + ". MBean may not be registered, or may not work properly.", e);
				return e;
			}
	...
	// a return value are simple getters, while void methods are modifiers. At
	// the
	...
	public synchronized void setAttribute(Attribute attrObj) throws AttributeNotFoundException, InvalidAttributeValueException, MBeanException, ReflectionException {
		try {
			...
			if (attr == "factoryClassLocation") // special case
			{
				...
				// else try treating factoryClassLocation like any other
				// attribute
				// on the presumption that some future, unexpected DataSource
				// that
				// exposes this property will not require the property to be set
				// at
			...
			if (rec == null)
				...
				if (!ai.isWritable())
					...
					// if we were unable to set this attribute directly in the
					// PooledDataSource,
					// we are updating a property of a nested DataSource, and we
					// should reset
					// the pool manager of the PooledDataSource implementation
					// so that these
					...
					if (target != pds) {
						if (pds instanceof AbstractPoolBackedDataSource)
							...
							logger.warning("MBean set a nested ConnectionPoolDataSource or DataSource parameter on an unknown PooledDataSource type. " + "Could not reset the pool manager, so the changes may not take effect. " + "" + "c3p0 may need to be updated for PooledDataSource type " + pds.getClass() + ".");
	...
	private static Map extractAttributeInfos(Object bean) {
		if (bean != null) {
			try {
				Map out = new HashMap();
				BeanInfo beanInfo = Introspector.getBeanInfo(bean.getClass(), Object.class); // so
																								// we
																								// don't
																								// see
																								// getClass()
				...
				for (int i = 0, len = pds.length; i < len; ++i) {
					...
					/*
					 * Note that it's not a problem that these getters and
					 * setters are not against this class the MBeanAttributInfo
					 * just uses the method names and attribute type to
					 * construct itself, and does not hold the methods
					 * themselves for future invocation.
					 */
